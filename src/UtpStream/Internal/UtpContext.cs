...
/// </summary>
internal unsafe sealed class UtpContext : IDisposable
{
    ...
    {
        SetCb(LibUtp.UTP_SENDTO, Marshal.GetFunctionPointerForDelegate(s_cbSendTo));
        SetCb(LibUtp.UTP_ON_READ, Marshal.GetFunctionPointerForDelegate(s_cbOnRead));
        SetCb(LibUtp.UTP_ON_STATE_CHANGE, Marshal.GetFunctionPointerForDelegate(s_cbOnStateChange));
        SetCb(LibUtp.UTP_ON_ERROR, Marshal.GetFunctionPointerForDelegate(s_cbOnError));
        SetCb(LibUtp.UTP_ON_FIREWALL, Marshal.GetFunctionPointerForDelegate(s_cbOnFirewall));
        SetCb(LibUtp.UTP_ON_ACCEPT, Marshal.GetFunctionPointerForDelegate(s_cbOnAccept));
        SetCb(LibUtp.UTP_GET_READ_BUFFER_SIZE, Marshal.GetFunctionPointerForDelegate(s_cbGetReadBufferSize));
        ...
        SetCb(LibUtp.UTP_GET_RANDOM, Marshal.GetFunctionPointerForDelegate(s_cbGetRandom));
        SetCb(LibUtp.UTP_GET_MICROSECONDS, Marshal.GetFunctionPointerForDelegate(s_cbGetMicroseconds));
        SetCb(LibUtp.UTP_GET_MILLISECONDS, Marshal.GetFunctionPointerForDelegate(s_cbGetMilliseconds));
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private void SetCb(int id, nint fp) => LibUtp.utp_set_callback(_ctx, id, fp);

    // Delegates stored as static fields so the GC never collects them — the
    // native thunks generated by GetFunctionPointerForDelegate remain valid
    // for the lifetime of the process.  Using managed-delegate callbacks
    // (rather than [UnmanagedCallersOnly] + delegate* unmanaged) lets the CLR
    // generate proper managed↔unmanaged transition code, which makes P/Invoke
    // calls from within the callbacks (utp_get_userdata, utp_read_drained, …)
    // safe under tiered JIT compilation.
    private static readonly UtpCallback s_cbSendTo = CbSendTo;
    private static readonly UtpCallback s_cbOnRead = CbOnRead;
    private static readonly UtpCallback s_cbOnStateChange = CbOnStateChange;
    private static readonly UtpCallback s_cbOnError = CbOnError;
    private static readonly UtpCallback s_cbOnFirewall = CbOnFirewall;
    private static readonly UtpCallback s_cbOnAccept = CbOnAccept;
    private static readonly UtpCallback s_cbGetReadBufferSize = CbGetReadBufferSize;
    private static readonly UtpCallback s_cbGetRandom = CbGetRandom;
    private static readonly UtpCallback s_cbGetMicroseconds = CbGetMicroseconds;
    private static readonly UtpCallback s_cbGetMilliseconds = CbGetMilliseconds;
